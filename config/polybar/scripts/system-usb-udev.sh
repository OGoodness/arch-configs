...
path_pid="/home/smallpotato/.config/polybar/system-usb-udev.pid"
...
case "$1" in
    ...
    --mount)
        ...
        for mount in $(echo "$devices" | jq -r '.blockdevices[] | select(.type == "part") | select(.rm == true) | select(.mountpoint == null) | select(.mountpoint != "/boot") | select(.mountpoint != "/") | .name'); do
        ...
        ;;
    --unmount)
        devices=$(lsblk -Jplno NAME,TYPE,RM,SIZE,MOUNTPOINT)

        for unmount in $(echo "$devices" | jq -r '.blockdevices[] | select(.size!="10M") | select(.type == "part") | select(.rm == true) | select(.mountpoint != null) | .name'); do
